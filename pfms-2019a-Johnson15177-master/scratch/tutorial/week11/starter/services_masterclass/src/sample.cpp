             ...
             a5_setup::RequestGoal::Response &res)
{
  ...
  //END
    ...
    std::cout << "global position (" << robotPose.position.x << ", " << robotPose.position.y << ")" << std::endl;
    
    double offset = (1*image.rows/resolution_)/2;
    
    double xpos = robotPose.position.x - req.x;
    double ypos = robotPose.position.y - req.y;
    
    std::cout << "local position (" << xpos << ", " << ypos << ")" << std::endl;
    // Compute values in pixels - we need to convert to an int
    int xP = static_cast<int>((xpos/resolution_) + offset);
    // Compute values in pixels - we need to convert to an int
    int yP = static_cast<int>((-ypos/resolution_) + offset);
    // origin is at robot - centre of map
    cv::Point org(image.rows/2, image.cols/2);
    // destination is at the request pixel
    cv::Point dest(xP, yP);
    ...
    while (ros::ok()) {
      ...
      //! What data do we need?
        if(goalReceived_)
        {
            ...
            //amd tjem apply a angular velocity to turn the robot
            //until the robot yaw of robot is such that it faces the point
            //attempt on your own and the discuss 
        }

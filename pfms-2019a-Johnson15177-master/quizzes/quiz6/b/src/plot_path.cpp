        ...
        while (ros::ok()) {
            ...
            if(imageOK){
                ...
                for(unsigned int i= 0; i < pathArrayBuffer_.poses.poses.size(); i++)
                {
                    ...
                    std::cout << "*** path elem: " << i << std::endl;
                    std::cout << "global position (x, Y) = (" << pose.position.x << ", " << pose.position.y << ")" << std::endl;
                    ...
                    std::cout << "local position (x, y) = (" << xpos << ", " << ypos << ")" << std::endl;
                    
                    double xD = (xpos/ pathArrayBuffer_.resolution) + offset;
                    double yD = (-ypos/ pathArrayBuffer_.resolution) + offset;
                    
                    std::cout << "path elem: " << i << "position (" << xD << ", " << yD << ")" << std::endl;
                    cv::Point pt(xD, yD);
                    cv::circle(rgbImage, pt, 3, CV_RGB(0, 0, 255), 1);          
                  ...
                  //! cv::circle(rgbImage, cv::Point((xD), (yD)), 3, CV_RGB(255,0,0),-1);
                   ...
                   if(client_.call(srv))
                   {
                    ROS_INFO("Response: %d", srv.response.ack);
                   }
                   else
                   {

             ...
             a5_setup::RequestGoal::Response &res)
{
  ...
  //ROS_INFO("sending back response: [%d]", res.ack)
    ...
    res.ack = false;
    std::cout << robotPose.position.y << std::endl;  
    ...
    //std::cout << "Sending back response: " << res.ack << std::endl;
...
void PfmsSample::seperateThread() {
    ...
    */
    ...
    //!
    //! Additional Question
    //! What is rate_limiter? it seems like the rate limiter will adjust the rate by dividing by a number to get the desired seconds
    //!
    ...
    while (ros::ok()) {
        ...
        if(!image.empty())
        {
            ...
            if(goalRequested_)
            {
                std::cout << "goal (x,y) = (" << goalPoint_.x << ", " << goalPoint_.y << ")" << std::endl;
                
                //ROS_INFO_STREAM("goal (x, y) = (" << goalPoint_.x << ", " << goalPoint_.y << ")");
                double offset = (image.cols)/2;
                std::cout << image.cols/2 << std::endl;
                goalPoint_.x = x;
                goalPoint_.y = y;
                
                std::cout << "local position (" << goalPoint_.x << ", " << goalPoint_.y << std::endl;
                ...
                std::cout << "image position (" << xD << ", " << yD << ")" << std::endl;             
